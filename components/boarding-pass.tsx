...
import {
  ...
  Ticket, Lock, CheckCircle2, Shield, Eye
} from 'lucide-react';
...
export function BoardingPass({ ticket }: { ticket: ETicketEntity }) {
  ...
  const { flight, bookingReference, seatNumber, passportNumber, passportExpiry } = ticket;
  ...
  return (
    <>
      <div className="rounded-xl overflow-hidden border-2 border-primary/20 bg-black/60 backdrop-blur-md shadow-2xl transition-all hover:border-primary/40 group max-w-2xl mx-auto w-full">
        <div className="flex flex-col md:flex-row">
          ...
          <div className="flex-1 p-6 border-b-2 md:border-b-0 md:border-r-2 border-dashed border-primary/30 relative overflow-hidden">
            ...
            {/* Passenger Passport Details (Shown if checked in) */}
            {(ticket.status === 'active' || ticket.status === 'boarded') && passportNumber && (
              <div className="mt-4 bg-white/5 rounded-xl p-3 border border-white/5 flex items-center justify-between relative z-10">
                <div className="flex items-center gap-2">
                  <Shield size={16} className="text-primary" />
                  <div>
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-mono">Passport Details Verified</p>
                    <p className="text-xs text-white font-mono mt-0.5">
                      No: {showPassport ? passportNumber : '••••••••'} (Exp: {passportExpiry})
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => setShowPassport(!showPassport)}
                  className="p-1 hover:bg-white/10 rounded transition-colors text-muted-foreground hover:text-white cursor-pointer"
                >
                  <Eye size={14} />
                </button>
              </div>
            ...
            <div className="mt-5 relative z-10">
              ...
              {/* CASE B: Window open — active check-in button triggers modal */}
              {checkInState.case === 'open' && (
                <div className="space-y-3">
                  <button
                    onClick={() => setIsModalOpen(true)}
                    className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-primary text-primary-foreground text-sm font-semibold font-mono tracking-wider hover:opacity-90 active:scale-[0.98] transition-all cursor-pointer"
                  >
      ...
      <CheckInModal
        ...
        onSuccess={() => {
          // Trigger any needed client-side notification/state update
          // The page will automatically revalidate because performCheckIn triggers revalidatePath
        }}
      />
    </>
  );
